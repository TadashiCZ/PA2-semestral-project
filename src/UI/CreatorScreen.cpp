...
void CreatorScreen::interact() {
	...
	cin.ignore();
	char dummyChar[200];
	cin.getline(dummyChar, 249);
	quiz.mName = string(dummyChar);
	mOs << COLOR_YELLOW << quiz.mName << "? Great name for a quiz. And what is your name?" << COLOR_RESET << endl;
	char dummyChar2[200];
	cin.getline(dummyChar2, 249);
	quiz.mAuthor = string(dummyChar2);
...
int CreatorScreen::loadTreeInteractive(Quiz & quiz) {
	...
	while ( pageCount <= 0 ) {
		...
		if ( isdigit( dummy[0] ) ) {
			...
			mOs << COLOR_YELLOW << "Input number." << COLOR_RESET << endl;
			continue;
		}
	...
	while ( num1 != 0 || dummy != ",x" ) {
		if ( cin >> num1 >> dummy ) {
			...
			mOs << COLOR_YELLOW << "Not like this! You broke it, oh god. Now I have to fix it again." << COLOR_RESET << endl;
	...
	for ( int i = 1 ; i < pageCount ; ++i ) {
		while ( num1 != i || c != ',' || num2 >= i ) {
			if ( cin >> num1 >> c >> num2 ) {
				if ( num1 == i && c == ',' && num2 < i ) {
					if (quiz.mPages[num2]->mBranches.size() < 3){
						...
						mOs << COLOR_YELLOW << "That page already has two children, that's enough. Try again." << COLOR_RESET << endl;
						continue;
					}
